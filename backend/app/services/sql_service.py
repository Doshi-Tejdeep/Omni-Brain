...
def process_sql_query(query: str) -> str:
    ...
    try:
        ...
        if "recent" in query and any(keyword in query for keyword in SESSION_KEYWORDS):
            sessions = get_recent_sessions(db)

            if not sessions:
                return "No recent sessions found."

            return "\n".join(f"- {session.title}" for session in sessions)
        ...
        if any(keyword in query for keyword in HISTORY_KEYWORDS):
            return "Please specify the session ID to retrieve " "chat history."
